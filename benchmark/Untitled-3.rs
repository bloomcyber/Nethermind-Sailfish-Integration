diff --git a/node/src/worker_rpc_client.rs b/node/src/worker_rpc_client.rs
index d89758289a9cb7851b1e7bc2c138934ec0880d6e..86dcd0d9c21d98a626c2379f2a32f572051ab7a4 100644
--- a/node/src/worker_rpc_client.rs
+++ b/node/src/worker_rpc_client.rs
@@ -1,137 +1,88 @@
...
-        .map(|x| x.to_string())
-        .collect::<Vec<_>>();
+    let worker_rpc = matches.value_of("RPC").unwrap().to_string();
...
+    let claims = Claims {
+        exp: (Utc::now() + ChronoDuration::minutes(5)).timestamp() as usize,
+    };
...
-    tokio::pin!(interval);
+    let client_filter = warp::any().map(move || (client.clone(), worker_rpc.clone(), token.clone()));
...
-    )?;
+async fn handle_tx(req: TxRequest, params: (reqwest::Client, String, String)) -> Result<impl warp::Reply, warp::Rejection> {
+    let (client, rpc, token) = params;
+    info!("Received transaction: {}", req.tx);
...
+    {
+        Ok(resp) => {
+            info!("Forwarded transaction to {}", rpc);
