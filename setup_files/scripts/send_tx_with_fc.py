...
if len(sys.argv) < 2:
    print("Usage: python3 send_tx_with_fc.py <host1:port> [host2:port …]")
    sys.exit(1)
...
def send_forkchoice_update(url, parent_hash, timestamp, txs, token):
    params = [
        {
    ...
    response = rpc_call(url, "engine_forkchoiceUpdatedV4", params, token)
    return response
...
def main():
    ...
    for url in ENDPOINTS:
        try:
            # Fetch current head block hash
            head_resp = rpc_call(url, "eth_getBlockByNumber", ["latest", False], token)
            parent_hash = head_resp["result"]["hash"]
            timestamp = int(time.time()) + 1

            # Send forkchoice update with transactions
            fc_resp = send_forkchoice_update(url, parent_hash, timestamp, batch_txs, token)
            print(f"Sent forkchoice update with batch {batch_key} to {url} | Payload ID: {fc_resp.get('result', {}).get('payloadId')}")
        except Exception as e:
            print(f"Error sending forkchoice update to {url}: {e}")
